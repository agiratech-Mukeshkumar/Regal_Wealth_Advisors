...
def login():
    """
    Handles the first step of login: email and password verification.
    If 2FA is enabled, it sends a code and returns a temporary token.
    """
    ...
    try:
        ...
        #  2FA is NOT enabled, log in 
            ...
            return jsonify({
                ...
                "token": final_token, "user": {"id": user['id'], "email": user['email'], "role": user['role']}
            })

    except Exception as e:
        conn.rollback(); return jsonify({"message": f"An error occurred: {e}"}), 500
    finally:
...
def verify_2fa():
    """
    Verifies the 6-digit code and returns the final JWT.
    """
    data = request.get_json()
